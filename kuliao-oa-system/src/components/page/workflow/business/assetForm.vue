<template>
    <div>
        ...
        <div class="container">
            <div class="form-box">
                <el-form :disabled="readonly" ref="ruleForm" :model="ruleForm" label-width="100px" :rules="rules" >
                    ...
                    <el-form-item label="领取理由" prop="applicationReason">
                        <el-input type="textarea" style="width: 60%" rows="5" v-model="ruleForm.applicationReason"></el-input>
                    </el-form-item>
                    <el-form-item label="附加信息" prop="remark">
                        <el-input type="textarea" style="width: 60%" rows="5" v-model="ruleForm.remark"></el-input>
                    </el-form-item>
                </el-form>
            </div>
        </div>
...
<script>
    export default {
        ...
        created() {
            console.log('我可读吗？'+this.readonly)
            this.getTypes();
            this.getEmp();
        },
        methods: {
            getEmp() {
                ...
                that.$axios.post('/base/user/query',params).then((res) => {
                    that.$set(that.ruleForm,'userId', res.data.data[0].empNo);
                    that.$set(that.ruleForm,'userName',res.data.data[0].username);
                    that.$set(that.ruleForm,'department',res.data.data[0].deptName);
            ...
            back(){
                let that = this;
                this.$router.push({path:'/leaveBill'});
            },
            //根据类型取到相应类型的固定资产列表
